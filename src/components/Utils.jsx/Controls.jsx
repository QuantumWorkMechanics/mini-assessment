...
function Controls({ handleNext, handleBack, isAutoAdvance, setIsAutoAdvance, questionIndex, isActive }) {
  const [tipOpen, setTipOpen] = useState(false);
  return (
    <>
      <div className=" flex mt-4 md:mt-0 justify-between md:justify-center ">
        <div className="p-2 ">
          {questionIndex == 0 && (
      ...
      <div className="md:hidden flex items-center gap-2 justify-center" data-tip="Will advance to next question when enough selections are made.">
        <label className="label cursor-pointer">
          <span className="label-text pr-3 ">Auto-advance</span>
          <input type="checkbox" className="toggle bg-[#09497B]" checked={isAutoAdvance} onChange={() => setIsAutoAdvance((prev) => !prev)} />
        </label>
