...
export default function SubComponent({ data }) {
  ...
  function getDataSet(data) {
    let tempData = tidy(data, groupBy("Dimension", [summarize({ Current: mean("Current"), Desired: mean("Desired") })]));
    return tempData;
  }
  ...
  return (
    <>
      ...
      <h2 className="ml-4 md:ml-40 mt-14 text-3xl font-light underline decoration-[#FDB517]">{data[0].Type}</h2>
      <div className="md:flex items-center justify-center xl:justify-around">
        {data && (
          <div>
            <div className=" mt-5">
              <Overall title="Current" total={returnAvg(data, "Current")} resultLookup={data[0].Type.toLowerCase()} />
            </div>
            <div className=" md:mt-40">
              <Overall title="Desired" total={returnAvg(data, "Desired")} resultLookup={data[0].Type.toLowerCase()} />
            </div>
          </div>
        )}
        <div className="flex flex-col">
          <div id={data[0].Type + "_bar"} className="w-[400px] h-[250px] md:h-[300px] md:w-[500px] mt-20 flex flex-col place-self-center">
            <SubBarGraph dataSet={dataSet} />
          </div>
          <div id={data[0].Type + "_radar"} className="block w-[400px] md:w-[600px] h-[180px] md:h-[300px] md:mt-72 md:ml-10">
