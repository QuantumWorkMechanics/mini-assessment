...
export default function Header({ currentQuestion, count, questionIndex }) {
  return (
    <>
      <div className="max-sm:flex max-sm:gap-2 max-sm:w-screen justify-between p-1">
        <div className="w-1/2 mt-2 md:-mb-1 md:ml-40  text-slate-600 text-sm md:text-sm">
          Question <span className="font-bold text-[#09497B]">{questionIndex + 1} </span>of <span className="font-bold text-[#09497B]">{count}</span>
        </div>
        <h3 className="md:hidden text-right font-bold text-xs min-h-4 overflow-auto mt-2">{currentQuestion.Dimension}</h3>
      </div>
